                                ...
                                if (inf.DevicePath.Contains("disk&ven_msft&prod_virtual_disk"))
                                {
                                    using (var disk = DiskHandle.OpenDisk(inf.DevicePath, false))
                                    {
                                        ...
                                        using (var mm = new SafePtr(sdisize))
                                        {
                                            ...
                                            r = (HResult)VDiskNative.GetStorageDependencyInformation(disk, GET_STORAGE_DEPENDENCY_FLAG.GET_STORAGE_DEPENDENCY_FLAG_HOST_VOLUMES | GET_STORAGE_DEPENDENCY_FLAG.GET_STORAGE_DEPENDENCY_FLAG_DISK_HANDLE, (uint)mm.Length, mm, out realsize);
